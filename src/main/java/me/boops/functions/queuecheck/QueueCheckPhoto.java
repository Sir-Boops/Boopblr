...
public class QueueCheckPhoto {
	...
	public QueueCheckPhoto(Long id, String blog_name) throws Exception {
		...
		String post_hash = new MD5Sum().hash(Cache.get_post_post.getJSONArray("posts").getJSONObject(0)
				.getJSONArray("photos").getJSONObject(0).getJSONObject("original_size").getString("url"));
		...
		while (new APIQueueCount().queue_count > scanned && !found) {
			...
			while (sub_runs < posts.length()) {
				...
				if (((JSONObject) posts.get(sub_runs)).get("type").equals("photo")) {

					// Calcucate The Post Hash And CHeck it!
					if (post_hash.equals(new MD5Sum().hash(posts.getJSONObject(sub_runs).getJSONArray("photos")
							.getJSONObject(0).getJSONObject("original_size").getString("url"))) && !found) {

						// Found A Duplicate!
						new Logger(post_hash + " : "
								+ new MD5Sum().hash(posts.getJSONObject(sub_runs).getJSONArray("photos")
										.getJSONObject(0).getJSONObject("original_size").getString("url")),
								0);
						found = true;
					} else {
