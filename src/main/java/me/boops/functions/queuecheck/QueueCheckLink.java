...
public class QueueCheckLink {
	...
	public QueueCheckLink(long id, String blog_name) throws Exception {
		...
		String post_hash = new MD5Sum().hash(decode.getPostLinkURL());
		...
		while (blog.getQueueCount() > scanned && !found) {
			...
			while (sub_runs < queue.getResPostCount()) {
				...
				if (decode.getPostType().equals("link")) {
					...
					if (post_hash.equals(new MD5Sum().hash(decode.getPostLinkURL())) && !found) {
						...
						// Nope Keep Chacking
						logger.Log(post_hash + " : " + new MD5Sum().hash(decode.getPostLinkURL()), 0, true);
						sub_runs++;
					}
				}
