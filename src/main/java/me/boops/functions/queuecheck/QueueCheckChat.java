...
public class QueueCheckChat {
	...
	public QueueCheckChat(long id, String blog_name) throws Exception {
		...
		String post_hash = new MD5Sum().hash(decode.getChatBody());
		...
		while (blog.getQueueCount() > scanned && !found) {
			...
			while (sub_runs < queue.getResPostCount()) {
				...
				if (decode.getPostType().equals("chat")) {
					...
					if (post_hash.equals(new MD5Sum().hash(decode.getChatBody())) && !found) {
						...
						// Nope Keep Chacking
						logger.Log(post_hash + " : " + new MD5Sum().hash(decode.getChatBody()), 0, true);
						sub_runs++;
					}
				}
